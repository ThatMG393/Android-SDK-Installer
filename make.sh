...
# Welcome to make.sh v9!
# 
# Features:
# Full argument support (since 'v6')
# Auto-detect project flavor (since 'v5') (removed since 'v9')
# Pass custom Gradle arguments (since 'v6')

VERSION="v9"
LAUNCHER_SCRIPT="asdk" 
# Make sure it doesn't have a prefix
...
make() {
    ...
    if [ ! -f $GEN_FLDR/*.jar ]; then err "Gradle did not generate any jar files"; fi
    ...
    JAR_FNAME=$( basename -- $GEN_FLDR/*.jar )
    info "Copying '${JAR_FNAME}' to '${BLD_MAIN_FLDR}'..."
    cp $GEN_FLDR/*.jar $BLD_MAIN_FLDR
    
    # JAR_VER=$( echo "$JAR_FNAME" | grep -o "\-[0-9]*.[0-9]*" )

    cp $LAUNCHER_SCRIPT $BLD_FLDR
    ...
    info "Creating ZIP..."
    zip -r $LAUNCHER_SCRIPT.zip $BLD_FLDR

    echo ""
    info "Moving '${LAUNCHER_SCRIPT}' to '${PKG_FLDR}'"
    mv $LAUNCHER_SCRIPT.zip $PKG_FLDR
}
